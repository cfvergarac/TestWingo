...
Abrir_Pagina_Principal
    ...
    ...    ELSE IF    ('${AMBIENTE}' == 'QALIN')   set test variable   ${IDIOMA}    'EN'
    ...    ELSE IF    ('${AMBIENTE}' == 'QAL')   set test variable   ${IDIOMA}    'ES'
...
Ingresa_Pasajeros
    ...
    Repeat Keyword    ${NUMADULTOS}       Click Javascript    ${ADULTOS}
    Repeat Keyword    ${NUMNINOS}         Click Javascript    ${NINOS}
    Repeat Keyword    ${NUMINFANTES}      Click Javascript    ${INFANTES}
...
Seleccionar_moneda
	...
	${presentpr}=     Run Keyword And Return Status    Element Should Be Visible    ${PopUpRutas}
    Run Keyword If    ${presentpr}    Click Element      ${BtnPopUpRutas}
    Click Javascript    ${SlcMONEDA}
    Capture Page Screenshot
    Run Keyword If  ('${MONEDA}'== 'COP')    Click Javascript    ${MONEDACOP}
    Run Keyword If  ('${MONEDA}'== 'USD')    Click Javascript    ${MONEDAUSD}
    ${presentpr}=     Run Keyword And Return Status    Element Should Be Visible    ${PopUpRutas}
    Run Keyword If    ${presentpr}    Click Javascript      ${BtnPopUpRutas}

Manejo_flex

    #${ExistePUFlex}=   Run Keyword And Return Status    Wait Until Element Is Visible     ${BtnCONTINUARFLEX}     15s
    #Run Keyword If  ${ExistePUFlex}    Click Element    ${BtnCONTINUARFLEX}

    Wait Until Page Contains Element    //label[@for='vuelo-ida-week-0']    15s

    ${ExistePUFlex}=       Run Keyword And Ignore Error    Click Element    ${BtnCONTINUARFLEX}
    #Wait Until Page Contains Element    ${RDBNOFLEX}
    #Click Javascript    ${RDBNOFLEX}

    ${ExistePPOtraruta}=   Run Keyword And Return Status    Wait Until Element Is Visible     //img[@alt='close']
    Run Keyword If  ${ExistePPOtraruta}    Click Element     //img[@alt='close']

    ${BundleDesplegado}=    Run Keyword And Ignore Error    CLick Element    (//div[@class='price-bundle'])[1]
    ${BundleDesplegado}=    Get From List     ${BundleDesplegado}    0

    Run Keyword If     ('${BundleDesplegado}' == 'FAIL')    Click Javascript    ${RDBVUELOIDA}
    Run Keyword If     ('${BundleDesplegado}' == 'FAIL')    CLick Element    (//div[@class='price-bundle'])[1]

    Run Keyword If  ('${TVUELO}' == 'RT')   BundlesRT

    Wait Until Page Contains Element    ${BtnCONTINUARPV}
    Click Javascript    ${BtnCONTINUARPV}

    ${popupAB}=   Run Keyword And Return Status    Wait Until Page Contains Element   //div[@class='bundle-text-selector-button-popup name-plan']
    Run Keyword If  ${popupAB}    Click Element    //div[@class='bundle-text-selector-button-popup name-plan']
...
Continuar
    ...
    #Wait Until Page Contains     ${TxtTarifas}    15s
	#wait until element is visible   ${LblPrecio}

BundlesRT
    Click Javascript      xpath=//label[@for='vuelo-regreso-week-0']

    Click Javascript      xpath=(//label[contains(@for,'radiobundle2')])[1]
    #Click Javascript     ${RDBVUELOVUELTA}
    #Click Javascript    ${RDBVUELOVUELTA}
    #Click Javascript    ${BUNDLEORIGINALVUELTA}
...
Ingresa_infantes
    ...
    \    Select From List By Label          ${LblANOINF}      2019
...
Selecciona_sillas
    Wait Until Page Contains     ${TxtSillas}    10s
    Run Keyword If  ('${TVUELO}' == 'RT')    Sillas_RT
    Run Keyword If  ('${TVUELO}' == 'OW')    Sillas_OW

Sillas_RT
    #wait until page does not contain    ${GIFAVION}
    Wait Until Page Contains    ${TxtSillas}   15s
    Wait Until Element Is Visible     ${BtnVUELOREGRESO}    15s
    #Set Focus To Element      ${BtnVUELOREGRESO}
    Click Javascript    ${BtnVUELOREGRESO}
    Sillas_OW

Sillas_OW
    Run Keyword If  ('${TIENENINOS}' == 'False')    Wait Until Page Contains Element   ${BtnCONTINUAROWQA}    15s
    Run Keyword If  ('${TIENENINOS}' == 'False')    Click Javascript     ${BtnCONTINUAROWQA}
    #debug
    Run Keyword If  ('${TIENENINOS}' == 'True')     Wait Until Page Contains Element   ${BtnCONTINUAROW}    15s
	...
	Run Keyword If    ${presentpu}    Sleep   4s
...
Pago_tarjeta_credito
    ...
    #debug
    #Wait Until Page Contains Element    ${BtnSEGURO}    7s
    #Execute Javascript     document.getElementById("noseguroviaje").click();
	Wait Until Page Contains Element    ${ChkTARJETACREDITO}    15s
	Sleep  2s
    click javascript   ${ChkTARJETACREDITO}
    #ver detalles
    Input Text    ${LblNUMEROTARJETA}        ${NumeroTarjeta}
    Select From List By Label    ${SELMES}   ${MesExpiracion}
    Select From List By Label    ${SELANO}   ${AnoExpiracion}
    ...
    Capture Page Screenshot
    Click Element    ${PAGARPSE}
    Wait Until Page Does Not Contain Element    xpath=//div[@class='cloud']     30s
    Capture Page Screenshot
    Wait Until Page Contains     ${txtCompra}      10s
    #Scroll Element Into View     ${txtConfirmacion}
    Execute Javascript    window.scrollTo(459, 552)
    Capture Page Screenshot
    ${PNR}=   Get Text    ${LblPNRTC}
    log to console    ${PNR}
    addToClipBoard    ${PNR}
    ${PNRAP}=   Get Text    ${PNRAPELLIDO}
    log to console    ${PNRAP}
    addToClipBoard    ${PNRAP}
...
asigna url RT
    ...
    ${URL}=    Catenate  SEPARATOR=/   ${URLBOOKING.${AMBIENTE}}   ${UCIUDADIDA}   ${UCIUDADVUELTA}   ${UFECHAIDA}   ${UFECHAVUELTA}   ${UADULTOS}   ${UINFANTES}    ${UNINOS}   0   ${UMONEDA}   0   0   # ${UMONEDA}    multicurrency
...
asigna url OW
    ...
    ${URL}=    Catenate  SEPARATOR=/   ${URLBOOKING.${AMBIENTE}}   ${UCIUDADIDA}   ${UCIUDADVUELTA}   ${UFECHAIDA}   ${UADULTOS}   ${UINFANTES}    ${UNINOS}   1   ${UMONEDA}   0   0  #  ${UMONEDA}   multicurrency
    set test variable   ${URL}
