...
#Pop up de flex
${PopUpFlex}         id=popup-noancillaries
...
Asignar elementos Pagina principal
    ...
    ${BtnCONTINUARFLEX}=     set variable if      ${LENGUA}=='ES'        xpath=//button[@class='bt-wingo-blue-pop-up'][contains(.,'Continuar')]       xpath=//button[@class='bt-wingo-blue-pop-up'][contains(.,'Continue')]
    set test variable       ${BtnCONTINUARFLEX}

    ${RDBNOFLEX}=     set variable if      ${LENGUA}=='ES'        xpath=//label[contains(.,'No, gracias')]       xpath=//label[contains(.,'No, thanks')]
     set test variable       ${RDBNOFLEX}

    ${RDBVUELOIDA}=      Set Variable          xpath=//label[@for='vuelo-ida-week-0']
    set test variable     ${RDBVUELOIDA}

    ${BUNDLEORIGINAL}=      Set Variable          xpath=//input[@id='vuelo-ida-week-0original']
    set test variable     ${BUNDLEORIGINAL}

    ${RDBVUELOVUELTA}=      Set Variable          xpath=//label[@for='vuelo-regreso-week-0']
    set test variable     ${RDBVUELOVUELTA}

    ${BUNDLEORIGINALVUELTA}=      Set Variable          xpath=//input[@id='vuelo-regreso-week-0original']
    set test variable     ${BUNDLEORIGINALVUELTA}
